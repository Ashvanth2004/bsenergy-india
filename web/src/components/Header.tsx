    ...
    <>
      <header
        className={cn(
          ...
          isScrolled
            ? "bg-[#0D0304]/95 backdrop-blur-md py-3.5 border-b border-white/10 shadow-2xl"
            : "bg-gradient-to-b from-[#0D0304]/95 via-[#0D0304]/50 to-transparent py-5"
        )}
      >
        <div className="max-w-7xl mx-auto flex items-center justify-between">
          {/* Left: Official Brand Logo */}
          <Link href="/" onClick={handleHomeClick} className="group flex items-center gap-3">
            <div className="relative w-11 h-11 flex items-center justify-center rounded-full bg-white/5 border border-white/10 p-0.5 group-hover:border-[#9E1218]/80 transition-all duration-300 shadow-[0_0_20px_rgba(158,18,24,0.35)] overflow-hidden">
              <Image
            ...
            <div className="flex flex-col">
              <span className="text-white font-black tracking-widest text-lg leading-tight uppercase group-hover:text-[#9E1218] transition-colors">
                BS ENERGY
              </span>
              <span className="text-[#E2E8F0] text-[10px] font-mono tracking-[0.25em] uppercase font-bold">
                INDIA
              </span>
            </div>
          </Link>
          ...
          <nav className="hidden xl:flex items-center gap-6">
            {navItems.map((item) => (
              <Link
                ...
                className={cn(
                  "text-xs font-mono tracking-wider font-semibold transition-colors py-1 relative after:content-[''] after:absolute after:bottom-0 after:left-0 after:w-0 after:h-[2px] after:bg-[#9E1218] hover:after:w-full after:transition-all after:duration-200",
                  pathname === item.href ? "text-[#9E1218] after:w-full" : "text-gray-300 hover:text-[#9E1218]"
                )}
              >
                {item.name}
              </Link>
          ...
          <div className="hidden md:flex items-center gap-3">
            ...
            <button
              ...
              className="group flex items-center gap-2 px-4 py-2 text-xs font-mono font-bold text-white bg-gradient-to-r from-[#9E1218] via-[#B81D24] to-[#7F0C12] hover:from-slate-100 hover:to-slate-200 hover:text-black rounded-sm transition-all duration-200 shadow-[0_0_20px_rgba(158,18,24,0.4)] hover:shadow-[0_0_20px_rgba(255,255,255,0.4)]"
            >
          ...
          <div className="flex md:hidden items-center gap-2">
            <button
              onClick={onOpenQuoteModal}
              className="px-3 py-1.5 text-xs font-mono font-bold text-white bg-[#9E1218] rounded-sm"
            >
              QUOTE
            </button>
            <button
      ...
      {mobileMenuOpen && (
        <div className="fixed inset-0 z-40 bg-[#0D0304]/98 backdrop-blur-xl pt-24 px-6 pb-12 flex flex-col justify-between md:hidden border-b border-white/10 animate-in fade-in slide-in-from-top-4 duration-200">
          <div className="flex flex-col gap-4">
            <div className="text-[10px] font-mono tracking-widest text-[#E2E8F0] uppercase mb-2 border-b border-white/10 pb-2">
              NAVIGATION / MENU
            </div>
            {navItems.map((item) => (
              <Link
                ...
                className={cn(
                  ...
                  pathname === item.href ? "text-[#9E1218]" : "text-gray-200 hover:text-[#9E1218]"
                )}
              >
                <span>{item.name}</span>
          ...
          <div className="flex flex-col gap-3 pt-6 border-t border-white/10">
            <button
              ...
              className="w-full py-3.5 bg-gradient-to-r from-[#9E1218] to-[#7F0C12] text-white font-mono font-bold text-sm text-center rounded-sm tracking-wider flex items-center justify-center gap-2 shadow-[0_0_20px_rgba(158,18,24,0.4)]"
            >
              GET A QUOTE
